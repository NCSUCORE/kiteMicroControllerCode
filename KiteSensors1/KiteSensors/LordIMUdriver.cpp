...
{
  //Serial3327.write(Serial3327.available());//TEST
...
uint8_t imuInitFromSaved(uint32_t baud, uint8_t fltrHeader)
{
  filterHeader = fltrHeader;
  Serial3327.begin(baud, SERIAL_8N1, 33, 27);
  return 0;
}
...
uint8_t imuNext(uint32_t *x, uint32_t *y, uint32_t *z, uint32_t *wx, uint32_t *wy, uint32_t *wz,
                uint32_t *ax, uint32_t *ay, uint32_t *az)
{
  //Declare variables
  uint8_t lenPayload;
  ...
  //Recieve and Transmit MIP
  while (imuSerialAvailable() < 54) { //54
    delay(1);
  }
  while (imuGetData() != 0x75) {
    while (imuSerialAvailable() < 54) { //54
      delay(1);
    }
  }
  MIPdata[MIPindex] = 0x75;
  if(MIPdata[MIPindex++] == 0x75) {
    ...
    if(MIPdata[MIPindex++] == 0x65) {
      ...
      if(MIPdata[MIPindex++] == filterHeader) {
        ...
        //Recieve third payload (linear acc)
        MIPdata[MIPindex] = imuGetData();//length
        len = MIPdata[MIPindex++] - 2;
        MIPdata[MIPindex++] = imuGetData();//cmd desc TODO
        for(int i = 0; i < len; i++) { //TODO
